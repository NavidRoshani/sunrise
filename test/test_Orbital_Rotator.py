...
def test_Orbital_Rotation():
    ...
    assert wfn_a.isclose(tq.simulate(tq.gates.X([0,1])+RA.compile(),variables=variables))
    ...
    assert wfn_b.isclose(tq.simulate(tq.gates.X([2,3])+RB.compile(),variables=variables))
    ...
    assert wfn_c.isclose(tq.simulate(objective=tq.gates.X([0,1])+RC.compile(),variables=variables))
    assert wfn_d.isclose(tq.simulate(objective=tq.gates.X([2,3])+RC.compile(),variables=variables))
    assert wfn_e.isclose(tq.simulate(objective=tq.gates.X([0,1])+RD.compile(),variables=variables))
    assert wfn_f.isclose(tq.simulate(objective=tq.gates.X([2,3])+RD.compile(),variables=variables))
